...
namespace EFCore
{
    public class ApplicationDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Post>().HasMany(p => p.Tags).WithMany(p => p.Posts).UsingEntity<PostTag>();
           ...
           /* modelBuilder.Entity<Post>().HasMany(p => p.Tags).WithMany(p => p.Posts).UsingEntity<PostTag>(
                j => j.HasOne(p => p.Tag).WithMany(p => p.PostTags).HasForeignKey(p => p.TagId),
                j => j.HasOne(p => p.Post).WithMany(p => p.PostTags).HasForeignKey(p => p.PostId),
                j =>
                {
                    j.Property(p => p.AddedOn).HasDefaultValueSql("GETDATE()");
